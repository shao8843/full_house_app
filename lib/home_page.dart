...
class OperationApp extends HookWidget with ServiceGetter {
  ...
  Widget build(BuildContext context) {
    ...
    useMemoized(
            () => changeUserOperation(
            mainMenu, snapshot.data?.role?.name, snapshot.data),
        [snapshot.data?.role?.name]);
    //add new menu
    return child;
  }
  ...
  void changeUserOperation(MenuGroup mainMenu, String? roleName, User? user) {
    if (roleName == null) {
      ...
      // mainMenu.addOrReplaceMenu(MenuItem(operationMenuName,
      //     widget: (_) => ImageIcon(AssetImage('assets/icons/app_icon.png')),
      //     widget2: widgetBuilder,
